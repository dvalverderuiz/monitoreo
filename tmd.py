...
def mostrar_tabla_dispositivos(devices):
...
def mostrar_tabla_interfaces(devices):
    for device in devices:
        if 'Interfaces' in device and device['Interfaces']:
            if_table = Table(title=f"Interfaces de {device.get('System Name', device['ip'])}", header_style="bold green")
...
def cli():
    parser = argparse.ArgumentParser(description="Escáner SNMP de dispositivos e interfaces")
    parser.add_argument("modo", choices=["usuarios", "interfaces"], help="Modo de ejecución: usuarios o interfaces")
    ...
    if not devices:
        ...
        if args.modo == "usuarios":
            mostrar_tabla_dispositivos(devices)
        elif args.modo == "interfaces":
            mostrar_tabla_interfaces(devices)
